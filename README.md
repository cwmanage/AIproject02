...
> 适用版本：2026-09-10（双训练方式版：传统机器学习 + PyTorch 深度学习）
> 项目性质：机器学习/深度学习课程作业（Titanic 生还预测 + 数据可视化 + Web 演示）

---

## 1. 这是什么

一个基于 **891 名泰坦尼克乘客数据** 的机器学习/深度学习演示系统，**同时提供两种训练方式**，顶部按钮一键切换（**主页为深度学习**）：

**🧠 深度学习页（主页 `/`，PyTorch MLP）**
- **6 张训练图表**：训练/验证 Loss 曲线、Accuracy 曲线、混淆矩阵、ROC 曲线、概率分布、六模型对比图
- **神经网络模型**：Linear(12→64)→ReLU→Dropout(0.2)→Linear(64→32)→ReLU→Linear(32→2)，共 2978 参数
- **训练配置**：CrossEntropyLoss + Adam(lr=0.001, weight_decay=1e-4)，batch=32，80 epochs，seed=42
- **网页交互演示**：填写乘客信息 → MLP 实时预测是否生还
- 划分：891 → 569 训练 / 143 验证 / 179 测试（与传统模型共用同一测试集，可直接对比）

**🧮 机器学习页（`/ml`）**
- **6 张数据可视化图**：生还分布、缺失值、性别/舱位生还率、年龄分布、票价分布 + 六模型对比
...
> 深度学习页另带 `/api/dl/summary`、`/api/dl/predictions`、`/api/dl/predict` 三个接口，
> 详细清单见《启动步骤》文档。
